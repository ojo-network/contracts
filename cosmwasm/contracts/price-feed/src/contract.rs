...
use crate::msg::QueryMsg::IsRelayer;
use crate::state::{RefData, ReferenceData, ADMIN, REFDATA, RELAYERS, RelayerStatus, RefStore};
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::UpdateAdmin { admin } => {
            ...
            check_if_admin(&prev_admin, &admin)?;
...
fn execute_add_relayers(
    ...
    check_if_admin(&admin, &info.sender)?;

    // Adds relayer
    for relayer in relayers {
        RelayerStatus::save(deps.storage, &deps.api.addr_validate(&relayer)?, &true)?;
    }
...
fn execute_remove_relayers(
    ...
    for relayer in relayers {
        RelayerStatus::save(deps.storage, &deps.api.addr_validate(&relayer)?, &false)?;
    }
...
fn query_is_relayer(deps: Deps, relayer: &Addr) -> StdResult<bool> {
    let status = RELAYERS.get(deps.storage, relayer).unwrap();
    Ok(status)
}
